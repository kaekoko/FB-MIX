...
const HeaderTab = ({ values, setFieldValue, categoryData }) => {
  ...
  const { data, isLoading } = useQuery(
    [product],
    () => request({ url: product, params: { status: 1 } }),
    ...
    <>
      <Row>
        <Col sm="9">
                ...
                <li key={i}>
                  <div className="selection-box">
                    <Input
                      ...
                      id={elem.id}
                      checked={
                        values["options"]?.["header"]?.["header_options"] ==
                        elem.value
                          ? true
                          : false
                      }
          ...
          <CheckBoxField
            name="[options][header][page_top_bar_dark]"
            title="TopbarDark"
          />
          <SimpleInputField
            nameList={[
              {
                ...
                title: "SupportNumber",
                placeholder: t("EnterSupportNumber"),
              },
            ]}
          />
          <MultiSelectField
            ...
            title="TodaysDeals"
            data={data}
            helpertext="*Choose products to display in the deal popup on the header. Select up to 3 recommended products."
          />
          <MultiSelectField
            values={values}
            setFieldValue={setFieldValue}
