...
import useAgentBlock from "@/Utils/Hooks/Agent/useblock-agent";
...
const Status = ({ url, data, disabled, apiKey }) => {
  ...
  const {
    isLoading,
    data: apiData,
    mutate: AgentBlockMutate,
  } = useAgentBlock(url, false, () => url);
  ...
  const handleClick = (value) => {
    setStatus(value);
    AgentBlockMutate({ id: data.id, status: Number(value) });
    setModal(false);
  };
  return (
    <>
